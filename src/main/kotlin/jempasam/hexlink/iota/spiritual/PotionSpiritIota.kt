...
class PotionSpiritIota(val blocktype: StatusEffect): Iota(TYPE, blocktype), SpiritIota {
    ...
    companion object{
        val TYPE=object : IotaType<PotionSpiritIota>(){
            ...
            override fun deserialize(tag: NbtElement, world: ServerWorld): PotionSpiritIota?{
                if(tag is NbtString){
                    var type=Registry.STATUS_EFFECT.getOrEmpty(Identifier(tag.asString())).orElseThrow(::IllegalArgumentException)
            ...
            override fun display(tag: NbtElement): Text {
                if(tag is NbtString){
                    var type=Registry.STATUS_EFFECT.getOrEmpty(Identifier(tag.asString()));
                    if(!type.isEmpty()){
