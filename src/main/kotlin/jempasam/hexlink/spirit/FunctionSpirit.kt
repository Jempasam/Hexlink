...
class FunctionSpirit(val manifestAt: Identifier, val manifestIn: Identifier, val manifestAtCost: Int, val manifestInCost: Int) : Spirit{
    ...
    override fun manifestAt(caster: LivingEntity?, world: ServerWorld, position: Vec3d, count: Int): Spirit.Manifestation {
        ...
        if(function.isEmpty)
            ...
            return Spirit.Manifestation(manifestAtCost, count){
                for(i in 0..<it){
                    val source=(if(caster!=null) caster.commandSource else world.server.commandSource).withPosition(position).withLevel(3)
    ...
    override fun lookIn(caster: LivingEntity?, world: ServerWorld, entity: Entity): Boolean = false
